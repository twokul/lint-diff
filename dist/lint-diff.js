...
if (process.env.CUSTOM_ESLINT_CONFIG_FILE) {
  linter = new _eslint.CLIEngine({ configFile: process.env.CUSTOM_ESLINT_CONFIG_FILE });
} else {
  linter = new _eslint.CLIEngine();
}
var formatter = linter.getFormatter();

var getChangedFiles = (0, _pipeP3.default)(function (commitRange) {
  return (0, _execa2.default)('git', ['diff', commitRange, '--name-only', '--relative', '--diff-filter=ACM']);
